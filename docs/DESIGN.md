...
The matching core is single-writer C++20 with ordered price maps, FIFO links in preallocated slots, a free-slot stack and an unordered ID index. Insert/cancel costs O(log L) price-level work plus expected O(1) ID lookup; matching consumes M makers with a conservative O(M log L) bound. Snapshot is O(N). Quote reads cached best-level aggregates in O(1); self-trade validation scans the makers a taker would reach. Two-limb unsigned aggregates preserve the standalone core's full uint64 quantity range: quote fails explicitly while a total is unrepresentable and recovers after fills/cancels reduce it. The account service's tighter bounds exclude that overflow. Containers and trade vectors still allocate. A failed allocation poisons the durable service and requires restart.
...
Threads, frame size, active order capacity, account count, event retention, event/book page size and socket deadlines are bounded. The OS owns its TCP queues. The journal is append-only and grows with fresh requests; operators must monitor disk space. Book snapshots are immutable and shared by version, retained for five seconds with a global 64 MiB order-storage budget and at most 32 versions. A full book is copied once when capturing a new version; each continuation copies only its page. Eviction/expiry returns changed=1. Response vectors and result buffers still allocate. Full risk/kill operations can hold the state lock longer than a simple matching command; this is not a hard real-time implementation.
