...
Book provides full resting depth. Start with offset=0, version=0; subsequent pages use the returned version and advance offset by count. If a mutation changed the global sequence, changed=1 and count=0; restart pagination. This may starve on a continuously busy book. Every page currently copies a full bounded snapshot under the mutex; large-depth subscribers should query infrequently. There is no pushed multicast or full-depth incremental feed.
