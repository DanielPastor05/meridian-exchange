...
import {directory,cleanup,start,delay} from '../tests/network_helpers.mjs';
const [executable,output]=process.argv.slice(2);if(!executable||!output)throw Error('usage: node tools/load.mjs SERVER OUTPUT_DIRECTORY');
fs.mkdirSync(output,{recursive:true});
const scenarios=[{rate:100,duration:2000},{rate:1000,duration:2000},{rate:10000,duration:2000},{rate:100000,duration:2000,burst:true}];
const results=[];
...
for(const durability of ['buffered','sync'])for(const scenario of scenarios){
 const dir=directory();let server;const clients=[];const rows=[];const latencies=[],wireLatencies=[],lateness=[];
 let sent=0,dropped=0,completed=0,failed=0,next=0,active=0;const sequences=[0,0,0],lastIds=[0,0,0];
 try{
  ...
  const submit=(i,planned)=>{
   const account=i%3,actual=performance.now()-begin,kind=i%2===0?1:2;
   if(busy[account]>=64){dropped++;rows.push([i,account+1,planned,actual,'','','shed']);return;}
   busy[account]++;active++;sent++;const sequence=++sequences[account];
   ...
   const promise=clients[account].submit(sequence,kind,id,1,kind===1?90:0,kind===1?1:0).then(reply=>{
    const done=performance.now()-begin;completed++;latencies.push((done-planned)*1000);wireLatencies.push((done-actual)*1000);lateness.push((actual-planned)*1000);
  ...
  const stem=durability+'-'+scenario.rate+(scenario.burst?'-burst':'');
